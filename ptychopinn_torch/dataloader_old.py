...
def npz_headers(npz):
    ...
    We can use this to determine shape of npz constituents without loading them
...
def fix_tensordict_memmap_state(tensordict, prefix):
    ...
    if tensordict._memmap_prefix is None:
        tensordict._memmap_prefix = prefix
...
class PtychoDataset(Dataset):
    ...
    def __init__(self, ptycho_dir: str, model_config: 'ModelConfig', data_config: 'DataConfig',
        ...
        # Try except because of distributed data parallel hang-up
        try:
        ...
        self.data_dict = {} # Initialize before calculate_length if it populates parts of it
        ...
        self.state_path = data_prefix_path / 'state_files.npz'

        # --- Coordinated Memory Map Creation/Loading (Rank 0 orchestrates) ---
        if self.current_rank == 0:
            ...
            # else:
                # print(f"[Rank 0] Decision: Load existing map (or files missing but not remaking).") # Optional
    ...
    def calculate_length(self):
        """
        The purpose of this function is to get the total number of diffraction patterns from a given dataset
        ...
        for i, npz_file in enumerate(self.file_list): # Use ordered list
            ...
            valid_indices_per_file.append(valid_indices) # Store for later use

            if n_valid_points == 0:
                print(f"Warning: No points found within bounds for file {npz_file}")
            # ---------------------------------

            # Check whether object.big is checked using stored config
    ...
    def memory_map_data(self, image_paths):
        ...
        for i, npz_file in enumerate(image_paths):
            ...
            if self.model_config.object_big: # Use stored config
                ...
                #Global coordinates with bounding, so we can pull from them
                #These are just arbitrary coordinates, so we're allowed to grab "coordinates"
                #that lie outside the bounding range. This is just for solution region translation
                ...
                #In channel = 1 case, our actual indices are only the valid ones from before
                #This is sync'd up 1:1 with the coordinates
            ...
            #Handles multiple probes if those dimensions are available
            if len(probe_data.shape) == 2:
                probe_data, _ = hh.normalize_probe(probe_data)
            ...
            #Center of mass coordinate. Not affected by solution region approach
            self.data_dict['com'] = torch.from_numpy(np.array([xcoords.mean(), ycoords.mean()]))

            #Object
            objectGuess = np.load(npz_file)['objectGuess']

            #Default no objectGuess is just a matrix of ones
            ...
            #NN_indices will be used to index within valid range so we do not need to pre-index.
            diff_stack = torch.from_numpy(np.load(npz_file)['diff3d']).to(torch.float32)
            ...
            #Get normalization constant from image stack
            print("Starting normalization process...")
            if self.data_config.normalize: # Use stored config
                # Pass necessary configs to helper function if needed
                norm_factor = hh.get_scaling_factor(diff_stack, self.data_config.N, self.data_config) # Pass N
                self.data_dict["scaling_constant"][i] = norm_factor
            else:
                norm_factor = 1
                self.data_dict["scaling_constant"][i] = norm_factor
            print(f"Finished normalizing. Normalizing constant: {norm_factor}")

            print(f"nn_indices dtype: {nn_indices.dtype}")
